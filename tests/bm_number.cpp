...
static void BM_ParseDouble_Zuu(benchmark::State& state) {
    for (auto _ : state) {
    ...
    state.counters["Parsed/s"] = benchmark::Counter(
        static_cast<double>(state.iterations() * double_strings.size()), benchmark::Counter::kIsRate);
}
BENCHMARK(BM_ParseDouble_Zuu)->Unit(benchmark::kNanosecond)->MinTime(1.0);

static void BM_ParseDouble_StdFromChars(benchmark::State& state) {
    for (auto _ : state) {
        for (const auto& str : double_strings) {
            double value;
...
static void BM_ParseInt_Zuu(benchmark::State& state) {
    for (auto _ : state) {
...
static void BM_ParseInt_StdFromChars(benchmark::State& state) {
    for (auto _ : state) {
        for (const auto& str : int_strings) {
            long long value;
            auto res = std::from_chars(str.data(), str.data() + str.size(), value);
