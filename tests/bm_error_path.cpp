...
static std::string generate_deep_nesting(size_t depth) {
    std::string json;
    json.reserve(depth * 2);
    for (size_t i = 0; i < depth; ++i) json += "[";
    for (size_t i = 0; i < depth; ++i) json += "]";
    return json;
}

// --- Data Uji Error Path ---
static const std::string valid_json = R"({"name": "zuu", "age": 21, "active": true, "tags": ["cpp", "json"]})";

// Kesalahan di akhir file (Trailing Comma) -> Menguji kecepatan bail-out di fase akhir
static const std::string err_trailing_comma = R"({"name": "zuu", "age": 21, "active": true, "tags": ["cpp", "json"],})";
...
static void BM_ErrorPath_ValidBaseline(benchmark::State& state) {
    for (auto _ : state) {
...
static void BM_ErrorPath_TrailingComma(benchmark::State& state) {
    for (auto _ : state) {
...
static void BM_ErrorPath_UnquotedKey(benchmark::State& state) {
    for (auto _ : state) {
...
static void BM_Adversarial_DeepNesting(benchmark::State& state) {
    for (auto _ : state) {
