...
Serializer::Serializer(const allocators::Storage* storage, int indent) noexcept
    ...
    if (storage) {
        estimated_size +=
            (storage->getArrayElementsCount() + storage->getObjectElementsCount()) * 24;
    }
...
std::string
    Serializer::dump(const allocators::Storage* storage,
...
void
    Serializer::writeIndent() noexcept {
    if (indent_ >= 0) {
        out_ += '\n';
...
void
    Serializer::serializeValue(const models::JsonValue& value) noexcept {
    switch (value.get_type()) {
        ...
        case enums::JsonType::Integer:
            serializeInteger(value.as_integer());
            break;
        case enums::JsonType::Double:
            serializeDouble(value.as_double());
            break;
        case enums::JsonType::String:
            serializeString(storage_->string(value.as_index()));
            break;
...
void
    Serializer::serializeString(std::string_view str) noexcept {
    out_ += '"';
...
void
    Serializer::serializeInteger(long long val) noexcept {
    char buf[32];
...
void
    Serializer::serializeDouble(double val) noexcept {
    char buf[64];
    auto [ptr, ec] = std::to_chars(buf, buf + sizeof(buf), val);
    if (ec == std::errc()) {
        out_.append(buf, ptr - buf);
...
void
    Serializer::serializeArray(const models::JsonValue& value) noexcept {
    out_ += '[';
...
void
    Serializer::serializeObject(const models::JsonValue& value) noexcept {
    out_ += '{';
