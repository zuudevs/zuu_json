...
void*
    Storage::allocate(uint32_t size, uint32_t alignment) noexcept {
    uint32_t align_offset =
        (alignment - (reinterpret_cast<uintptr_t>(tail_->data.get() + tail_->used) % alignment)) %
        alignment;
    ...
    if (tail_->used + align_offset + size > tail_->capacity) {
        ...
        tail_ = tail_->next;
        align_offset =
            (alignment - (reinterpret_cast<uintptr_t>(tail_->data.get()) % alignment)) % alignment;
    }
...
bool
    Storage::hasRoot() const noexcept {
    return root_set_;
}

void
    Storage::setRoot(models::JsonValue value) noexcept {
    root_ = value;
    root_set_ = true;
}

const models::JsonValue&
    Storage::root() const noexcept {
    return root_;
}

uint64_t
    Storage::commitString(std::string_view value) noexcept {
    strings_.push_back(value);
    return strings_.size() - 1;
}

char*
    Storage::allocateStringBuffer(uint64_t length) noexcept {
    return static_cast<char*>(allocate(length, 1));
}

uint64_t
    Storage::sealArray(const models::JsonValue* elements, uint32_t count) noexcept {
    ...
    auto* ptr = static_cast<models::JsonValue*>(
        allocate(count * sizeof(models::JsonValue), alignof(models::JsonValue)));
...
uint64_t
    Storage::sealObject(const models::JsonMember* members, uint32_t count) noexcept {
    ...
    auto ptr = static_cast<models::JsonMember*>(
        allocate(count * sizeof(models::JsonMember), alignof(models::JsonMember)));
...
bool
    Storage::isObjectSorted(uint64_t index) const noexcept {
    return objects_[index].is_sorted;
}

void
    Storage::sortAllObjects() noexcept {
    for (auto& meta : objects_) {
        if (!meta.is_sorted && meta.size > 1) {
            ...
            std::ranges::sort(
                ...
                [this](const models::JsonMember& member) { return resolveKey(member); });
            meta.is_sorted = true;
        }
    }
}
...
Storage::JsonArray
    Storage::array(uint64_t index) const noexcept {
    const auto& meta = arrays_[index];
    return {meta.first, meta.second};
}

Storage::JsonObject
    Storage::object(uint64_t index) const noexcept {
    const auto& meta = objects_[index];
    return {meta.ptr, meta.size};
}

std::string_view
    Storage::string(uint64_t index) const noexcept {
    return strings_[index];
}

std::string_view
    Storage::resolveKey(const models::JsonMember& member) const noexcept {
